...
func (t *ServiceInfo) PageList(c *gin.Context, tx *gorm.DB, param *dto.ServiceListInput) ([]ServiceInfo, int64, error) {
	total := int64(0)                             // 总数
	list := []ServiceInfo{}                       //储存当前页数据
	offset := (param.PageNo - 1) * param.PageSize //计算偏移量（跳过前N条记录）

	query := tx.WithContext(c)                              //绑定请求上下文
	query = query.Table(t.TableName()).Where("is_delete=0") //查询未删除的记录
	if param.Info != "" {
		// 模糊查询,?是gorm的占位符，%是SQL的通配符，所以这里要使用%+param.Info+%
		//最终生成的sql条件是WHERE (service_name LIKE '%支付%' OR service_desc LIKE '%支付%')
...
func (t *ServiceInfo) Find(c *gin.Context, tx *gorm.DB, search *ServiceInfo) (*ServiceInfo, error) {
	...
	if result.RowsAffected == 0 {
		// 没找到记录，返回 gorm.ErrRecordNotFound
